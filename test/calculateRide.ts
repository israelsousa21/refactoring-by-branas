...
test("Deve calcular o valor da corrida em horário no domingo", function () {
    const fare = calculateRide([{ distance: 10, date: new Date("2022-06-12T10:00:00") }]);
    expect(fare).toBe(29);
});
...
test("Deve retornar -2 se a data for inválida", function () {
    expect(() => calculateRide([{ distance: 10, date: new Date("xxxxxxx") }])).toThrow(new Error("Invalid Date"))
});

test("Deve calcular uma corrida em multiplos horários", function () {
